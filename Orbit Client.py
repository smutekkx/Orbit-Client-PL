...
class OrbitWindowBrandingEngine:
    ...
    def _boost_cpu_priority(self):
        try:
            handle = win32api.OpenProcess(
                win32con.PROCESS_SET_INFORMATION, True, self.target_pid
            )
    ...
    def monitor_target(self):
        for _ in range(200):
            try:
                win32gui.EnumWindows(self._process_windows, None)
            except:
                pass
            time.sleep(0.2)
    ...
    def _process_windows(self, hwnd, lparam) -> bool:
        if win32gui.IsWindowVisible(hwnd):
            ...
            if pid == self.target_pid:
                ...
                if (
                    "LWJGL" in w_class
                    or "GLFW30" in w_class
                    or w_class == "SunAwtFrame"
                ):
                    if win32gui.GetWindowText(hwnd) != self.expected_title:
                        win32gui.SetWindowText(hwnd, self.expected_title)
                    if self.hicon:
                        if (
                            ...
                            win32gui.SendMessage(
                                hwnd, win32con.WM_SETICON, win32con.ICON_BIG, self.hicon
                            )
        return True
...
class OrbitSkinCacheManager:
    def __init__(self):
        self.cache_dir = os.path.join(BASE_FOLDER, "cache", "skins")

    def get_player_head(self, username: str, callback: Callable[[Any], None]):
        threading.Thread(
            target=self._fetch_skin_flow, args=(username, callback), daemon=True
        ).start()
    ...
    def _fetch_skin_flow(self, username: str, callback: Callable[[Any], None]):
        ...
        try:
            res = requests.get(
                f"https://api.mojang.com/users/profiles/minecraft/{username}", timeout=2
            )
            if res.status_code == 200:
                user_id = res.json().get("id")
                p_res = requests.get(
                    f"https://sessionserver.mojang.com/session/minecraft/profile/{user_id}",
                    timeout=2,
                )
                if p_res.status_code == 200:
                    ...
                    skin_bytes = requests.get(
                        decoded["textures"]["SKIN"]["url"]
                    ).content
                    with open(
                        os.path.join(self.cache_dir, f"{username}_full.png"), "wb"
                    ) as f:
                        f.write(skin_bytes)
                    img = Image.open(
                        os.path.join(self.cache_dir, f"{username}_full.png")
                    )
                    img.crop((8, 8, 16, 16)).resize(
                        (45, 45), Image.Resampling.NEAREST
                    ).save(target_path)
                    callback(target_path)
                    return
        except:
...
class OrbitLogWatcher:
    def __init__(self, console_callback: Callable[[str, str], None]):
        self.console_callback = console_callback

    def start_watch(self, process: subprocess.Popen):
        threading.Thread(target=self._watch_loop, args=(process,), daemon=True).start()
...
class OrbitLunarLauncher(ctk.CTk):
    def __init__(self):
        ...
        self.user_nick = self.config_manager.settings.get(
            "last_used_profile", "_smutek"
        )
        ...
        # Automatyczny raport przy starcie
                ...
                for line in remote_code.split("\n"):
                    if "VERSION_LAUNCHER =" in line:
                        ...
                        if remote_version != VERSION_LAUNCHER:
                            ...
                            messagebox.showinfo(
                                "Orbit Updater",
                                f"Wykryto nową wersję ({remote_version})! Launcher zaktualizował się automatycznie i uruchomi się ponownie.",
                            )
    ...
    def build_launcher_interface(self):
        ...
        self.sidebar_frame = ctk.CTkFrame(
            self, width=100, fg_color=BG_PANEL, corner_radius=0
        )
        self.sidebar_frame.grid(row=0, column=0, sticky="nsew")
        ctk.CTkLabel(
            self.sidebar_frame, text="🌙", font=("Arial", 38), text_color=ACCENT
        ).pack(pady=35)
        ...
        self.profile_top_bar = ctk.CTkFrame(
            self.main_container, fg_color="transparent", height=60
        )
        self.profile_top_bar.grid(row=0, column=0, sticky="ne", pady=(0, 10))

        self.top_nick_lbl = ctk.CTkLabel(
            self.profile_top_bar,
            text=self.user_nick,
            font=("Segoe UI", 14, "bold"),
            text_color=TEXT_LIGHT,
        )
        self.top_nick_lbl.pack(side="left", padx=10)
        self.top_avatar_lbl = ctk.CTkLabel(
            self.profile_top_bar,
            text="👤",
            font=("Arial", 24),
            text_color=ACCENT,
        ...
        self.viewport = ctk.CTkFrame(
            self.main_container, corner_radius=16, fg_color=BG_DARK
        )
    ...
    def _update_top_avatar(self, path: Optional[str]):
        if path and os.path.exists(path):
            ...
            self.top_avatar_lbl.configure(image="", text="👤")
    ...
    def render_dashboard_view(self):
        self.clear_viewport()
        header = ctk.CTkFrame(
            self.viewport, height=120, fg_color=BG_PANEL, corner_radius=16
        )
        header.pack(fill="x", pady=(0, 20))
        header.pack_propagate(False)
        ctk.CTkLabel(
            header, text="ORBIT CLIENT", font=("Impact", 44), text_color=TEXT_LIGHT
        ).pack(side="left", padx=35, pady=25)
        ctk.CTkLabel(
            header, text="v1.0 (r)", font=("Segoe UI", 20), text_color=TEXT_LIGHT
        ).pack(side="left", padx=35, pady=25)
        ...
        self.v_dropdown = ctk.CTkOptionMenu(
            card,
            values=self.all_versions,
            width=360,
            height=42,
            fg_color=BG_DARK,
            button_color=ACCENT,
            command=lambda c: setattr(self, "selected_version", c),
        )
        ...
        self.engine_btn = ctk.CTkButton(
            card,
            text=f"Silnik startowy: {self.mode}",
            width=360,
            height=42,
            fg_color="#1b1e26",
            hover_color="#242934",
            command=self._toggle_loader,
        )
        self.engine_btn.pack(pady=15)

        self.status_bar = ctk.CTkLabel(
            card,
            text=f"Gotowy do uruchomienia",
            font=("Segoe UI", 13, "bold"),
            text_color="#10b981",
        )
        ...
        ctk.CTkButton(
            ...
            command=lambda: threading.Thread(
                target=self.execute_minecraft_engine, daemon=True
            ).start(),
        ).pack(pady=20)
    ...
    def render_mods_view(self):
        ...
        ctk.CTkLabel(
            self.viewport,
            text="Menedżer Modyfikacji Fabric",
        ...
        search_box = ctk.CTkEntry(
            filter_frame,
            placeholder_text="Wpisz nazwę moda i wciśnij Enter...",
            width=400,
            height=45,
            fg_color=BG_PANEL,
            border_color="#262936",
        ...
        mod_v_dropdown = ctk.CTkOptionMenu(
            filter_frame,
            values=self.mod_versions_list,
            width=120,
            height=42,
            fg_color=BG_PANEL,
            button_color=ACCENT,
            command=lambda c: setattr(self, "mod_search_version", c),
        )
        mod_v_dropdown.set(self.mod_search_version)
        mod_v_dropdown.pack(side="left")

        scroll = ctk.CTkScrollableFrame(
            self.viewport, fg_color=BG_PANEL, corner_radius=15
        )
        ...
        def run_search(e=None):
            ...
            try:
                res = requests.get(
                    f'https://api.modrinth.com/v2/search?query={search_box.get()}&facets=[["categories:fabric"],["versions:{self.mod_search_version}"]]'
                ).json()
                for item in res["hits"]:
                    ...
                    ctk.CTkButton(
                        ...
                        fg_color=ACCENT,
                        width=100,
                        command=lambda m=item: self._download_mod(
                            m, self.mod_search_version
                        ),
        ...
        try:
            ...
            for v in v_data:
                if version_str in v["game_versions"] and "fabric" in v["loaders"]:
                    with open(
                        os.path.join(INSTANCE_DIR, "mods", v["files"][0]["filename"]),
                        "wb",
                    ) as f:
    ...
    def render_packs_view(self):
        ...
        ctk.CTkLabel(
            top_frame,
            text="Menedżer Paczek Zasobów (Txt)",
            font=("Segoe UI", 26, "bold"),
            text_color=TEXT_LIGHT,
        ).pack(side="left")
        ctk.CTkButton(
            ...
            hover_color="#242934",
            font=("Segoe UI", 13, "bold"),
            command=lambda: os.startfile(PACKS_DIR),
        ...
        ctk.CTkButton(
            ...
            font=("Segoe UI", 13, "bold"),
            width=100,
            command=self.render_packs_view,
        ).pack(side="right", padx=5)
    ...
    def _load_local_resource_packs(self):
        ...
        if not files:
            ctk.CTkLabel(
                self.p_scroll,
                text="Folder resourcepacks jest pusty. Wrzuć tu pliki .zip paczek zasobów!",
                font=("Segoe UI", 14),
                text_color=TEXT_MUTED,
            ).pack(pady=40)
            return
        for filename in files:
            ...
            ctk.CTkLabel(row, text="📦", font=("Arial", 20), text_color=ACCENT).pack(
                side="left", padx=15
            )
            ctk.CTkLabel(
                row,
            ...
            ctk.CTkButton(
                ...
                width=80,
                height=32,
                command=lambda f=filename: self._delete_resource_pack(f),
            ).pack(side="right", padx=15)

    def _delete_resource_pack(self, filename):
        if messagebox.askyesno(
            "Orbit Resource Packs",
            f"Czy na pewno chcesz trwale usunąć paczkę {filename}?",
        ):
            try:
    ...
    def render_settings_view(self):
        ...
        self.ram_lbl = ctk.CTkLabel(
            card,
            text=f"Przydzielony RAM: {self.ram_val} GB",
    ...
    def render_console_view(self):
        ...
        ctk.CTkLabel(
            self.viewport,
            text="Konsola błędów (Sprawdź tu, jeśli gra się crashuje)",
            font=("Segoe UI", 26, "bold"),
            text_color=TEXT_LIGHT,
        ).pack(anchor="w", padx=25, pady=20)
